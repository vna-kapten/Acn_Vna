...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-2xl mx-auto">
        ...
        <form method="POST" action="{{ route('admin.books.update', $book) }}" enctype="multipart/form-data" class="bg-white shadow-md rounded-lg p-6">
            @csrf
            @method('PUT')

            <div class="mb-4">
                <label for="title" class="block text-sm font-medium text-gray-700">Book Title</label>
                <input type="text" name="title" id="title" value="{{ old('title', $book->title) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500" required>
                @error('title')
            ...
            <div class="mb-4">
                ...
                <select name="author_id" id="author_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500" required>
                    ...
                    @foreach($authors as $author)
                        <option value="{{ $author->author_id }}" {{ old('author_id', $book->author_id) == $author->author_id ? 'selected' : '' }}>{{ $author->author_name }}</option>
                    @endforeach
                </select>
                @error('author_id')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            ...
            <div class="mb-4">
                ...
                <select name="publisher_id" id="publisher_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500" required>
                    ...
                    @foreach($publishers as $publisher)
                        <option value="{{ $publisher->publisher_id }}" {{ old('publisher_id', $book->publisher_id) == $publisher->publisher_id ? 'selected' : '' }}>{{ $publisher->publisher_name }}</option>
                    @endforeach
                </select>
                @error('publisher_id')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            ...
            <div class="mb-4">
                ...
                <select name="shelf_id" id="shelf_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500" required>
                    ...
                    @foreach($shelves as $shelf)
                        <option value="{{ $shelf->shelf_id }}" {{ old('shelf_id', $book->shelf_id) == $shelf->shelf_id ? 'selected' : '' }}>{{ $shelf->shelf_name }}</option>
                    @endforeach
                </select>
                @error('shelf_id')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            ...
            <div class="mb-4">
                ...
                <textarea name="description" id="description" rows="3" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">{{ old('description', $book->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="number" name="book_quantity" id="book_quantity" value="{{ old('book_quantity', $book->book_quantity) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500" required>
                @error('book_quantity')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                @if($book->book_img)
                    <p class="mt-1 text-sm text-gray-500">Current image: {{ $book->book_img }}</p>
                @endif
                @error('book_img')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div class="flex justify-end">
                <a href="{{ route('admin.books.show', $book) }}" class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded mr-2">Cancel</a>
